...
HectorForCTPPS::HectorForCTPPS(const edm::ParameterSet & param, bool verbosity,bool CTPPSTransport) : 
    ...
    m_CTPPSTransport(CTPPSTransport)
...
void HectorForCTPPS::add( const HepMC::GenEvent * evt ,const edm::EventSetup & iSetup) {
	...
	if ( (*eventParticle)->status() == 1 && (*eventParticle)->pdg_id()==2212 ){
		...
		if ( m_beamPart.find(line) == m_beamPart.end() ) {
		    ...
		    if (fCrossAngleCorr) LorentzBoost(const_cast<TLorentzVector&>(p_out),"LAB");
...
void HectorForCTPPS::ApplyBeamCorrection(TLorentzVector& p_out)
{

    double microrad = 1.e-6;
    double theta = p_out.Theta(); if (p_out.Pz()<0) theta=TMath::Pi()-theta;
    double dtheta_x = (double)(m_smearAng)?CLHEP::RandGauss::shoot(0.,m_sigmaSTX):0;
    double dtheta_y = (double)(m_smearAng)?CLHEP::RandGauss::shoot(0.,m_sigmaSTY):0;
    double denergy  = (double)(m_smearE)?CLHEP::RandGauss::shoot(0.,m_sig_e):0.;
...
void HectorForCTPPS::LorentzBoost(TLorentzVector& p_out, const string& frame)
{
    // Use a matrix
    double microrad = 1.e-6;
